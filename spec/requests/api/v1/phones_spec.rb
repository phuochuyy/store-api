...
RSpec.describe "Api::V1::Phones", type: :request do
  ...
  describe "GET /api/v1/phones" do
    ...
    it "filters phones by brand" do
      ...
      other_phone = create(:phone, brand: other_brand, category: category)
    ...
    it "searches phones by name" do
      iphone = create(:phone, name: "iPhone 15", brand: brand, category: category)
  ...
  describe "PUT /api/v1/phones/:id" do
    ...
    context "as customer" do
      it "returns forbidden" do
        put "/api/v1/phones/#{phone.id}", params: update_params, headers: { "Authorization" => "Bearer #{customer_token}" }
        expect(response).to have_http_status(:forbidden)
      end
    end
  end
