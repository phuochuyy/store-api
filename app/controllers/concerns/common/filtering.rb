module Common::Filtering
  ...
  def apply_filters(collection, allowed_filters)
    allowed_filters.each do |filter_key, filter_value|
      ...
      case filter_key
      when :search
        collection = apply_search(collection, filter_value)
      when :date_range
        collection = apply_date_range(collection, filter_value)
      when :price_range
        collection = apply_price_range(collection, filter_value)
      else
  ...
  def apply_search(collection, search_term)
