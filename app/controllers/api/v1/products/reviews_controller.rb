    ...
    module Products
      class ReviewsController < Api::V1::BaseController
        ...
        def paginate_reviews(reviews)
          reviews.page(params[:page]).per(params[:per_page] || 10)
        end

        def build_reviews_response
          {
            reviews: @reviews.map { |review| review_serializer(review) },
            pagination: build_reviews_pagination
          }
        end

        def build_reviews_pagination
          {
        ...
        def review_serializer(review)
          {
            id: review.id,
            user: serialize_review_user(review.user),
            product: serialize_review_product(review.product),
        ...
        def serialize_review_product(product)
