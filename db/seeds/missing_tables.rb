...
if Discount.exists? && User.exists?
  ...
  if Order.exists?
    orders = Order.where(status: ['delivered', 'shipped', 'confirmed']).limit(3)
...
if Order.exists? && PaymentMethod.exists?
  orders = Order.where(status: ['delivered', 'shipped', 'confirmed', 'pending']).limit(10)
  ...
  orders.each do |order|
    ...
    begin
      ...
      conn.execute(
        "INSERT INTO payment_histories (payment_id, action, new_status, amount, transaction_id, gateway_response, performed_by, performed_at, notes, created_at, updated_at) 
         VALUES (#{payment.id}, 'create', #{conn.quote(payment.status)}, #{payment.amount}, #{payment.transaction_id ? conn.quote(payment.transaction_id) : 'NULL'}, #{payment.gateway_response ? conn.quote(payment.gateway_response) : 'NULL'}, 'System', #{conn.quote(payment.created_at)}, 'Payment created via seed data', #{conn.quote(now)}, #{conn.quote(now)})"
      )
      
      # Add status change history if payment is completed
      if payment.completed?
        conn.execute(
          "INSERT INTO payment_histories (payment_id, action, previous_status, new_status, amount, transaction_id, performed_by, performed_at, notes, created_at, updated_at) 
           VALUES (#{payment.id}, 'status_change', 'pending', 'completed', #{payment.amount}, #{payment.transaction_id ? conn.quote(payment.transaction_id) : 'NULL'}, 'System', #{conn.quote(payment.processed_at)}, 'Payment processed successfully', #{conn.quote(now)}, #{conn.quote(now)})"
        )
      end
...
payment_history_count = ActiveRecord::Base.connection.execute("SELECT COUNT(*) FROM payment_histories").first[0] rescue 0
...
if Product.exists?
  ...
  products.sample(20).each do |product|
    movement_types = ['in', 'out', 'adjustment', 'return', 'damage', 'transfer']
